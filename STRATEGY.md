...
Today VendorProof is a parallel fan-out of `exa.searchAndContents` (`type:auto`, `highlights`, `summary`, `includeDomains`). It does not use date filtering, livecrawl, the Research API, or Monitors. State that plainly; an Exa exec who opens `src/lib/exa.ts` will check. The expansion path, clearly aspirational:
